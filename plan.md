...
- [x] Schedule shows only the latest available tournament round, defaults to natural Match ID order, and supports Match, Date, and Referee sorting; Date also sorts each day's times.
- [x] Lower-bracket `a`-`d` potential rows remain available for signup until the numeric match is finalized, then collapse into the numeric ID with the selected candidate's referee assignment.
- [x] Admin-only shadcn schedule editor updates match date/time with calendar selection and validated `HH:MM` input; dates are written as native Sheets serial values so the existing cell format is retained.
